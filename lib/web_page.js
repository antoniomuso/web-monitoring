...
class WebPage extends EventEmitter {
  ...
  constructor (uri, options) {
    super()
    this.sanOpt = options.sanitizeOpt ?  options.sanitizeOpt : { allowedAttributes: {} }
    this.opt = {
      uri: uri,
      headers: {
    ...
    this.control = (options && options.whileControl && typeof options.whileControl === 'function')
            ...
            : (oldPage, newPage) => (leven(oldPage, sanitizeHtml(newPage,this.sanOpt)) / oldPage.length) <= options.percentageDiff
